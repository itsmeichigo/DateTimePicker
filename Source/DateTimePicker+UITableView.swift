...
extension DateTimePicker: UITableViewDataSource, UITableViewDelegate {
  ...
  public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
    ...
    if tableView == amPmTableView {
      ...
      if is12HourFormat {
        cell.textLabel?.text = String(format: "%01i", (indexPath.row % 12) + 1)
      } else {
        cell.textLabel?.text = String(format: "%01i", indexPath.row % 24)
      }
    }
  ...
  func adjustedRowForInfiniteScrolling(tableView: UITableView, selectedRow: Int) -> Int {
    ...
    } else if (tableView == minuteTableView || tableView == secondTableView),
      selectedRow > 60 * 3 - 1 {
